...
<p> <c:out value="Parcourir une liste d'étudiants"> </c:out>
</p>

<c:forEach items="${listeEtudiants}" var="etudiant" varStatus="k">
	<p> 
		N° <c:out value="${k.count}"> </c:out>  <c:out value="${etudiant}"> </c:out>
	</p>
</c:forEach>


<c:forEach items="${exemples}" var="exemple" begin="1" end="4" varStatus="j">
	<c:out value="${j.count}"></c:out> : <c:out value="${exemple}"></c:out>
</c:forEach>


<c:forTokens var="exemple" items="Salut je test" delims=";">
	<c:out value="${exemple}"></c:out>
</c:forTokens>
